...
WSS_URL = os.getenv("WSS_URL", "<YOUR_WEBSOCKET_URL_HERE>")
S3_BUCKET_NAME = os.getenv("S3_BUCKET_NAME", "S3_BUCKET_NAME")
OPENAI_MODEL = os.getenv("OPENAI_MODEL", "gpt-4o-mini")
WEBSOCKET_SHARED_SECRET="<WEBSOCKET_SHARED_SECRET_API_KEY>"
...
def get_processed_answer_from_image_threaded(image_bytes):
    ...
    try:
        ...
        prompt_text = (
            "You are an image-intelligence assistant. Given this OCR’d text:\n"
            "{image_text}\n"
            "Return a structured JSON with whatever you extract."
            )
...
def on_message(ws, message):
    ...
    try:
        ...
        if action == "processS3Image":
            ...
            if image_bytes:
                 threading.Thread(target=get_processed_answer_from_image_threaded, args=(image_bytes,), daemon=True).start()
...
class HostAppGUI:
    def __init__(self, master):
        self.master = master
        master.title("Image Processor Host Listener v2.1 (Secured)")
